...
class BlogIndexRepository{
    ...
    async selectByPlate(plate_name){
         ...
         const plate = await knex("blog").select('blog.id',  'blog.name', 'blog.comments', 'blog.rating').innerJoin("plates", "blog.plate_id", "plates.id").whereLike("plates.name", `%${plate_name}%`).orderBy("created_at"); 
